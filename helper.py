...
def fetch_stats(selected_user, df, keywords=None):
    #total messages in the grp

    #smaller function of below written code
    ...
    #fetch no of media
    num_media = df[df['messages'] == '\u200eimage omitted\n'].shape[0]

    #fetch no of links present
    links = []
    for messages in df['messages']:
...
def emoji_helper(selected_user, df, emoji):
    if selected_user != 'Overall':
        df = df[df['users'] == selected_user]
